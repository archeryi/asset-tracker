...
class AssetTracker {
    ...
    showActionSheet(title, actions) {
        return new Promise(resolve => {
            ...
            const bgHandler = (e) => { if (e.target === overlay) { cleanup(); resolve(-1); } };
    ...
    renderDashboard() {
        ...
        document.getElementById('category-count').textContent = [...new Set(this.data.accounts.map(a => a.category))].length;
        ...
        if (latest) {
            ...
            document.getElementById('category-list').innerHTML = '<div class="empty-state"><div class="empty-state-text">添加账户并记录第一笔快照</div></div>';
        }
    }
    ...
    renderTrendChart(snapshots) {
        ...
        if (!snapshots.length) {
            this.charts.trend = new Chart(ctx, { type:'line', data:{labels:[''],datasets:[{data:[0],borderColor:'#0a84ff'}]}, options:{responsive:true,plugins:{legend:{display:false}},scales:{x:{display:false},y:{display:false}}} });
            return;
        }
        this.charts.trend = new Chart(ctx, {
            type: 'line',
            data: { labels: snapshots.map(s=>s.date), datasets: [{ label:'总资产', data:snapshots.map(s=>this.getSnapshotTotal(s)), borderColor:'#0a84ff', backgroundColor:'rgba(10,132,255,0.1)', fill:true, tension:0.4, pointRadius: snapshots.length>15?1:3, pointHoverRadius:5 }] },
            options: { responsive:true, plugins:{ legend:{display:false}, tooltip:{ callbacks:{ label:(i)=>'¥'+i.raw.toLocaleString() } } }, scales:{ x:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#636366',maxTicksLimit:6}}, y:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#636366',callback:v=>'¥'+(v/10000).toFixed(0)+'万'}} } }
        });
    }
    ...
    renderAllocationChart(snapshot) {
        ...
        const ct = {};
        this.data.accounts.forEach(a => { const v = snapshot.assets[a.id]||0; if(v>0) ct[a.category]=(ct[a.category]||0)+v; });
        ...
        this.charts.allocation = new Chart(ctx, {
            ...
            options:{ responsive:true, cutout:'68%', plugins:{ legend:{position:'bottom',labels:{color:'#8e8e93',padding:10,font:{size:11}}}, tooltip:{callbacks:{label:(i)=>{const t=i.dataset.data.reduce((a,b)=>a+b,0);return `${i.label}: ¥${i.raw.toLocaleString()} (${((i.raw/t)*100).toFixed(1)}%)`;}}} } }
        });
    }
    ...
    renderCategoryList(snapshot) {
        ...
        c.innerHTML = sorted.map(([n,v],i) => `<div class="category-item"><div class="category-item-left"><div class="category-dot" style="background:${CATEGORY_COLORS[i%CATEGORY_COLORS.length]}"></div><span class="category-name">${n}</span></div><div><span class="category-value">${this.formatMoney(v)}</span><span class="category-pct">${total>0?((v/total)*100).toFixed(1):0}%</span></div></div>`).join('');
    }
    ...
    renderAccounts() {
        ...
        if (!this.data.accounts.length) {
            list.innerHTML = `<div class="ios-group" style="cursor:pointer" onclick="app.showAddAccount()"><div class="empty-state"><div class="empty-state-icon">+</div><div class="empty-state-text">添加第一个资产账户</div></div></div>`;
            return;
        }
        const grouped = {};
    ...
    showAddAccount(editId = null) {
        ...
        const opts = this.data.categories.map(c => `<option value="${c}" ${a&&a.category===c?'selected':''}>${c}</option>`).join('');
        this.openSheet(title, `
            <div class="form-group"><label>账户名称</label><input type="text" id="input-account-name" placeholder="如：招商银行储蓄卡" value="${a?a.name:''}" autocomplete="off"></div>
            <div class="form-group"><label>资产类别</label><select id="input-account-category">${opts}</select></div>
    ...
    // ==================== Snapshot ==================== 
    setSnapshotDate() { document.getElementById('snapshot-date').value = new Date().toISOString().split('T')[0]; }
            ...
            <div class="ios-group">
                ...
                ${accs.map(a => {
                    ...
                    return `<div class="snapshot-input-row">
                        ...
                        <input class="snapshot-input-field" type="number" inputmode="decimal" data-account-id="${a.id}" data-category="${cat}" placeholder="金额" value="${pv||''}" oninput="app.updateSnapshotTotals()">
    ...
    updateSnapshotTotals() {
        ...
        const sub = {}; let gt = 0;
        inputs.forEach(i => { const v = parseFloat(i.value)||0; sub[i.dataset.category]=(sub[i.dataset.category]||0)+v; gt+=v; });
        Object.entries(sub).forEach(([c,s]) => { const el = document.querySelector(`[data-subtotal="${c}"]`); if(el) el.textContent = this.formatMoney(s); });
    ...
    saveSnapshot() {
        ...
        const assets = {}; let has = false;
        document.querySelectorAll('.snapshot-input-field').forEach(i => { const v = parseFloat(i.value)||0; assets[i.dataset.accountId]=v; if(v>0) has=true; });
        ...
        if (idx >= 0) {
            this.showActionSheet(`${date} 的快照已存在`, [
                { label: '覆盖', destructive: true, action: () => {
                    ...
                    this.saveData(); this.toast('已保存','success'); this.renderSnapshotForm();
                }}
            ]);
        } else {
            this.data.snapshots.push({ date, assets });
            this.data.snapshots.sort((a,b)=>a.date.localeCompare(b.date));
            this.saveData(); this.toast('快照已保存','success'); this.renderSnapshotForm();
        }
    }

    // ==================== History ==================== 
    renderHistory() { this.updateHistoryChart(); this.renderHistoryList(); }
    ...
    updateHistoryChart() {
        ...
        if (range > 0) { const co = new Date(); co.setMonth(co.getMonth()-range); snaps = snaps.filter(s=>s.date>=co.toISOString().split('T')[0]); }
        const ctx = document.getElementById('chart-history');
        if (this.charts.history) this.charts.history.destroy();
        if (!snaps.length) {
            this.charts.history = new Chart(ctx,{type:'line',data:{labels:[''],datasets:[{data:[0],borderColor:'#0a84ff'}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{x:{display:false},y:{display:false}}}});
            return;
        }
        const ds = [{ label:'总资产', data:snaps.map(s=>this.getSnapshotTotal(s)), borderColor:'#0a84ff', backgroundColor:'rgba(10,132,255,0.08)', fill:true, tension:0.4, borderWidth:2, pointRadius:snaps.length>15?0:2 }];
        const cd = {};
        snaps.forEach(s => { this.data.accounts.forEach(a => { if(!cd[a.category]) cd[a.category]={}; if(!cd[a.category][s.date]) cd[a.category][s.date]=0; cd[a.category][s.date]+=(s.assets[a.id]||0); }); });
        Object.entries(cd).forEach(([cat,dm],i) => { const d=snaps.map(s=>dm[s.date]||0); if(d.some(v=>v>0)) ds.push({label:cat,data:d,borderColor:CATEGORY_COLORS[i%CATEGORY_COLORS.length],tension:0.4,borderWidth:1.5,pointRadius:0,borderDash:[4,4]}); });
        this.charts.history = new Chart(ctx, {
            type:'line', data:{labels:snaps.map(s=>s.date),datasets:ds},
            options:{responsive:true,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{color:'#8e8e93',font:{size:11}}},tooltip:{callbacks:{label:i=>`${i.dataset.label}: ¥${i.raw.toLocaleString()}`}}},scales:{x:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#636366',maxTicksLimit:6}},y:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#636366',callback:v=>'¥'+(v/10000).toFixed(0)+'万'}}}}
        });
    }
    ...
    renderHistoryList() {
        ...
        if (!snaps.length) { c.innerHTML='<div class="empty-state" style="padding:20px"><div class="empty-state-text">暂无记录</div></div>'; return; }
    ...
    deleteSnapshot(date) {
        this.showActionSheet(`删除 ${date} 的快照`, [
            { label: '删除', destructive: true, action: () => {
                ...
                this.saveData(); this.renderHistory(); this.toast('已删除');
            }}
        ]);
    }
    ...
    showAddCategory() {
        this.openSheet('添加类别', `
            <div class="form-group"><label>类别名称</label><input type="text" id="input-category-name" placeholder="如：数字货币" autocomplete="off"></div>
    ...
    addCategory() {
        ...
        this.data.categories.push(n); this.saveData(); this.renderSettings(); this.closeSheet();
        this.toast('已添加','success');
    }
    deleteCategory(name) {
        if (this.data.accounts.some(a=>a.category===name)) { this.toast('该类别下有账户，无法删除','error'); return; }
            ...
            try {
                ...
                this.showActionSheet('导入将覆盖现有数据',[
                    { label:'确认导入', destructive:true, action:()=>{
                        this.data=d; this.saveData(); this.init(); this.toast('导入成功','success');
                    }}
                ]);
            } catch(err) { this.toast('导入失败：'+err.message,'error'); }
        };
